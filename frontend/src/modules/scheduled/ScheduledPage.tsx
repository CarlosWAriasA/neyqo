...
const scheduledSchema = z
  ...
  .superRefine((value, context) => {
    ...
    if (
      ...
      value.firstDayOfMonth === value.secondDayOfMonth
    ) {
      context.addIssue({
        code: 'custom',
...
export function ScheduledPage() {
  ...
  const [selectedScheduled, setSelectedScheduled] =
    useState<ScheduledMovement | null>(null);
  const queryClient = useQueryClient();
  const scheduledQuery = useQuery({
    queryKey: ['scheduled-movements'],
    queryFn: getScheduledMovements,
  });
  ...
  const activeCategories = (categoriesQuery.data ?? []).filter(
    (category) =>
      category.status === 'active' && category.type === selectedType,
  );
  ...
  const scheduled = useMemo(() => {
    ...
    return (scheduledQuery.data ?? []).filter((item) => {
      ...
      const matchesQuery =
        !normalizedQuery ||
        [item.name, item.category, item.account, item.description ?? '']
          .join(' ')
          .toLowerCase()
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      toast.success(
        selectedScheduled
          ? 'Programado actualizado correctamente.'
          : 'Programado creado correctamente.',
      );
  ...
  const pauseMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        getScheduledErrorMessage(error, 'No pudimos pausar el programado.'),
      ),
  });
  ...
  const resumeMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        getScheduledErrorMessage(error, 'No pudimos reactivar el programado.'),
      ),
  });
  ...
  const deactivateMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        getScheduledErrorMessage(error, 'No pudimos desactivar el programado.'),
      ),
  });
      ...
      <Card className="grid gap-4">
        <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
          <label className="relative">
            <Search
              className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-subtle"
              aria-hidden="true"
            />
            <Input
              className="pl-9"
          ...
          <Select
            value={filter}
            onChange={(event) =>
              setFilter(event.target.value as ScheduledFilter)
            }
          >
      ...
      <div className="rounded-panel border border-border bg-surface/40 p-3 lg:min-h-0 lg:flex-1">
        <div className="category-scrollbar max-h-[68vh] overflow-y-auto pr-2 lg:h-full lg:max-h-none">
          ...
          {scheduledQuery.isLoading ? (
            <Card>Cargando programados...</Card>
          ) : null}
        ...
        <div>
          ...
          <p
            className={cn(
              ...
              item.type === 'income' ? 'text-positive' : 'text-danger',
            )}
          >
...
function ScheduledPanel({
  ...
  form: ReturnType<
    typeof useForm<ScheduledFormValues, unknown, ScheduledFormSubmitValues>
  >;
    ...
    return (
      <div className="grid gap-4 md:grid-cols-2">
        <Field
          ...
          error={form.formState.errors.firstDayOfMonth?.message}
        >
          <Input
            type="number"
            min="1"
            max="31"
            {...form.register('firstDayOfMonth')}
          />
        </Field>
        <Field
          label="Segundo día del mes"
          error={form.formState.errors.secondDayOfMonth?.message}
        >
          <Input
            type="number"
            min="1"
            max="31"
            {...form.register('secondDayOfMonth')}
          />
        </Field>
      ...
      <div className="ml-auto grid h-full w-full max-w-3xl content-start gap-5 overflow-y-auto rounded-panel bg-surface p-5 shadow-panel">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-subtle">
              {statusCopy[item.status].description}
            </p>
          </div>
          <Button
            variant="ghost"
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <DetailItem
            label="Tipo"
            value={item.type === 'income' ? 'Ingreso' : 'Gasto'}
          />
          ...
          <DetailItem
            label="Fecha final"
            value={item.endDate ? formatDate(item.endDate) : 'Sin fecha final'}
          />
          <DetailItem label="Estado" value={statusCopy[item.status].label} />
          {item.description ? (
            <DetailItem label="Descripción" value={item.description} />
          ) : null}
        </div>
        ...
        <Card className="grid gap-4">
          <div>
            ...
            <p className="text-sm text-subtle">
              Movimientos creados automáticamente desde este programado.
            </p>
          </div>
          {loading ? (
            <p className="text-sm text-subtle">Cargando historial...</p>
          ) : null}
...
function toScheduledPayload(
  values: ScheduledFormSubmitValues,
...
function buildPreview(params: {
  ...
  const start = params.startDate
    ? ` a partir del ${formatDate(params.startDate)}`
    : '';
...
function summarizeUpcoming(
  upcoming: Array<{ type: 'income' | 'expense'; amount: number }>,
) {
...
async function refreshScheduledQueries(
